...
public class Run {
	...
	public static void main(String[] args) {
		...
		card.getPayAmount();   // 부모 클래스(PaymentMethod)에 정의된 멤버
		...
		// [3] 따라서, 컴파일러는 부모 타입의 참조변수로 자식 인스턴스를 가리키는 것을 '타입 안전'하다고 판단한다.
		...
		//pm1.getCardNumber(); // => 컴파일 오류 발생! 실제 힙 메모리에는 생성되지만, 참조변수(pm1)는 알 수 없으므로 오류 발생!!
		// TODO: 위 오류 해결하여 이메일 제출! 6/22 23:59까지!
		...
		// PaymentMethod p1 = new CreditCardPay();
		arr[1] = new PointPay();
		...
		// * 다운 캐스팅 : 업캐스팅으로 인해 제한된 자식 고유의 기능을 사용하기 위해, 자식 타입으로 강제로 형변환하는 것.
		//   => 자식 타입으로의 변환은 힙 메모리에 해당 영역이 존재하는 지 컴파일러가 확신할 수 없으므로,
